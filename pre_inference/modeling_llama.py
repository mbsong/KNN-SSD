...
print('(Re-)Loading modeling LLaMA...')
...
class LlamaAttention(_LlamaAttention):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = LlamaRotaryEmbedding(self.head_dim, base=getattr(self.config, 'rope_theta', 10000), max_position_embeddings=self.max_position_embeddings)
        else:
            scaling_type = self.config.rope_scaling["type"]
            scaling_factor = self.config.rope_scaling["factor"]
            if scaling_type == "linear":
                self.rotary_emb = LlamaLinearScalingRotaryEmbedding(
                    self.head_dim, base=getattr(self.config, 'rope_theta', 10000), max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor
                )
...
class LlamaModel(_LlamaModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        # [MODIFIED] add swift mask when verification
