...
CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m evaluation_llama.inference_baseline --model-path $LLAMA_PATH --model-id ${MODEL_NAME} --max-new-tokens ${MAX_NEW_TOKENS} --task-name ${TASK_NAME} --sub-task-name ${SUB_TASK_NAME} --data-num ${DATA_NUM} --mix-ratio ${MIX_RATIO} --temperature ${TEMP} --top-p ${TOP_P} --seed ${SEED} --dtype ${torch_dtype}
